...
gulp.task(
	...
	gulp.series("prepare-fonts", () =>
		gulp.src([".tmp/fonts/**/*"]).pipe(gulp.dest("dist/fonts"))
	)
);

gulp.task("prepare-langs", () =>
	gulp.src(["src/langs/**/*"]).pipe(gulp.dest(".tmp/langs"))
);

gulp.task(
	"process-langs",
	gulp.series("prepare-langs", () =>
		gulp.src([".tmp/langs/**/*"]).pipe(gulp.dest("dist/langs"))
	)
);
...
gulp.task(
	...
	gulp.series("prepare-styles", "prepare-scripts", "prepare-views", () =>
		gulp
			...
			.pipe(
				$.replace(
					/url\((\.\.\/fonts\/[a-zA-Z0-9-]+\.woff2)(\?[a-zA-Z0-9\-_=.]+)?\)/g,
					(_match, fileName) => {
...
gulp.task(
	...
	gulp.series("process-html", () =>
		gulp
			.src([".tmp/css/**/*", ".tmp/js/**/*"], { base: ".tmp" })
			.pipe(gulp.dest("dist"))
	)
);

gulp.task("prepare-assets-bundle", () =>
	gulp.src(["src/favicon.png"]).pipe(gulp.dest(".tmp"))
);
...
gulp.task(
	...
	gulp.series(
		...
		"prepare-langs",
		"prepare-html",
		"prepare-assets-bundle",
		() =>
			gulp
				.src(".tmp/index.html")
				.pipe(
					$.replace(
						/<link rel="stylesheet" href="(css\/[a-zA-Z0-9\-_.]+\.css)">/g,
						(_match, fileName) => {
				...
				.pipe(
					$.replace(
						/<script src="(js\/[a-zA-Z0-9\-_.]+\.js)"><\/script>/g,
						(_match, fileName) => {
							const content = fs.readFileSync(`.tmp/${fileName}`, "utf8");
				...
				.pipe(
					$.replace(
						/url\(\.\.\/(fonts\/[a-zA-Z0-9-]+\.woff)(\?[a-zA-Z0-9\-_=.]+)?\)/g,
						(match, fileName) => {
				...
				.pipe(
					$.replace(
						/<link rel="icon" href="([a-zA-Z0-9\-_.]+\.png)">/g,
						(match, fileName) => {
							if (!fs.existsSync(`.tmp/${fileName}`)) {
								return match;
							}
				...
				.pipe(
					$.replace("<!-- AriaNg-Bundle:languages -->", () => {
						...
						if (fileNames.length > 0) {
							...
							for (const fileName of fileNames) {
								...
								const languageName = fileName.substring(0, lastPointIndex);
				...
				.pipe(
					$.replace(
						/<[a-z]+( [a-z-]+="[a-zA-Z0-9\- ]+")* [a-z-]+="((favicon.ico)|(tileicon.png)|(touchicon.png))"\/?>/g,
						""
					)
				)
				.pipe(gulp.dest("dist"))
...
gulp.task("process-full-extras", () =>
	gulp
		.src(["LICENSE", "src/*.*", "!src/*.html"], {
			dot: true,
		})
		.pipe(gulp.dest("dist"))
);

gulp.task("process-tiny-extras", () =>
...
gulp.task(
	"build-bundle",
	gulp.series("process-assets-bundle", "process-tiny-extras", "info")
);

gulp.task(
	"serve",
	gulp.series("prepare-styles", "prepare-scripts", "prepare-fonts", () => {
		browserSync({
			notify: false,
			port: 9000,
			server: {
